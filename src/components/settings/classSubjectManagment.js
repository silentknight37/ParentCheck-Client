        ...
        return fetch(`setting/getStudentEnroll?classId=${this.state.classId}&academicYear=${this.state.academicYear}`, requestOptions)
            ...
            .then(response => {
                response.studentEnrolls.map(i =>
                    classStudentList.push({ id: i.id, studentUserName: i.studentUserName, studentUserId: i.studentUserId, className: i.className, academicYear: i.academicYear, classId: i.classId, action: <Link className="btn btn-light" onClick={() => this.selectedTemplate(i)}><i className="icofont icofont-ui-note"></i></Link> })
                )
                this.setState({
...
export default ClassSubjectStudentManagment;